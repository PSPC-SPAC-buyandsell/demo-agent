...
def asc2decstr(a: str) -> str:
    return a if a.isdigit() else str(int.from_bytes(b2a_hex(a.encode()), "big"))
...
class BaseAgent:
    ...
    def __init__(self, pool: NodePool, seed: str, wallet_name: str, wallet_config: str) -> None:
        ...
        self._claim_req_json = None  # prover; FIXME: support multiple schema, use dict: txn_no -> claim_req_json
    ...
    async def verify_proof(self, proof_req: dict, proof: dict, schema: dict, claim_def: dict) -> str:  # verifier
        """
...
class ListeningAgent(BaseAgent):
    ...
    def schema_metadata(self, value):
        self._schema_metadata = value
    ...
    def _schema_and_claim_def_info(self, form_data: dict) -> (str, int, str):
        """
        Gets schema, seq no, claim def for use in indy-sdk structures, reading it from cached property
        or from the ledger.

        :param form_data: form['data'] from request form
        :return: tuple with json schema, int schema sequence (txn) number on the ledger, json claim def
        """
        ...
        return schema_json, schema_seq_no, claim_def_json
    ...
    async def process_get_txn(self, int txn) -> str:
        """
        Takes a request from the service wrapper to find a transaction on the distributed ledger,
        returns json with its sequence number or null if there is no match

        :param req: request form on which to operate
    ...
    async def process_get_did(self) -> str:
        """
        Takes a request from the service wrapper request to get DID, returns json with it
