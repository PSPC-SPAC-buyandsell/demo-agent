...
class BaseAgent:
    ...
    async def store_claim_req(
        ...
        :param master_secret: prover master secret (label)
    ...
    async def create_proof(self, proof_req: dict, schema: dict, master_secret: str, claim_def: dict) -> str:  # prover
        ...
        :param proof_req: proof request as verifier creates; has entries for proof request's nonce, name, version;
            ...
            E.g., {
                ...
                'requested_attrs': {
                    'attr1_uuid': {
                        ...
                        'encoded-value': (encoded value to match)
                    },
                    'attr2_uuid': {
                        'schema_seq_no': 57,
                        'name': 'height'
        ...
        :param master_secret: master secret (label)
        ...
        vals = {proof_req['requested_attrs'][uuid]['name']: proof_req['requested_attrs'][uuid].pop('encoded-value')
            for uuid in proof_req['requested_attrs']}
        ...
        for attr_uuid in claims_for_proof['attrs']:
            for candidate in claims_for_proof['attrs'][attr_uuid]:
                ...
                if vals.items() <= candidate['attrs'].items():
                    claim_uuid = candidate['claim_uuid']
                    print("\n\n^^^ SK GOT CLAIM UUID {}".format(claim_uuid))
                    break
        ...
        # print("\n\n^^^ SK PROOF {}\n".format(json.dumps(json.loads(rv), indent=4)))
        return rv
    ...
    async def verify_proof(self, proof_req: dict, proof: dict, schema: dict, claim_def: dict) -> bool:  # verifier
        ...
        :param proof_req: proof request as verifier creates (see above)
    ...
    async def get_claims_for_proof_req(self, proof_req_json: str, filter_enc: dict = None) -> str:  # prover
        """
        Method for prover to get claims (from wallet) corresponding to proof request

        :param proof_req_json: proof request json as verifier creates (see above)
        ...
        if filter_enc is not None:
            ...
            for pred_uuid in claims_for_proof['predicates']:
                for candidate in claims_for_proof['predicates'][pred_uuid]:
                    if candidate['claim_uuid'] not in claim_uuids:
                        claims_for_proof['predicates'][pred_uuid].pop(candidate)

        return json.dumps(claims_for_proof)
    ...
    async def process_post(self, req: dict) -> str:
        ...
        if req['type'] == 'schema-init':
            self.__class__._vet_keys(
                ...
                set(req['data'].keys()),
                hint='data')
            rv = self.send_schema(json.dumps({k: req['data'][k] for k in req['data'] if k in keys}))
            schema = json.loads(rv)
            ...
            return self.send_claim_def(schema)
        ...
        # in progress
        elif req['type'] == 'proof-request':
            self.__class__._vet_keys({'claim-filter'}, set(req['data'].keys()), hint='data')

            if 'issuer-did' in req['data']:
                schema_seq_no = json.loads(await self.get_schema(
                    req['data']['issuer-did'],
                    req['data']['schema-name'],
                    req['data']['schema-version']
                ))['seqNo']
            ...
            by_attr = {
                ...
                'requested_attrs': {
                    '{}_uuid'.format(attr): {
                        ...
                        'name': attr
                    } for attr in req['data']['claim-filter']
                },
                'requested_predicates': {
                }
            }
            claims_by_attr = await cag.get_claims_for_proof_req(json.dumps(by_attr))
            


        elif req['type'] == 'proxy-proof-request':
            self.__class__._vet_keys({'remote-did', 'claim-filter'}, set(req['data'].keys()), hint='data')
            endpoint = json.loads(get_endpoint_attrib(req['data']['remote-did']))
            (host, port) = tuple(endpoint['ha'].split(':'))
            #TODO: post get_claim to next hop, ... work it ... get proof
            return json.dumps({})
